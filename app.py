...
def load_data():
    ...
    df = pd.read_sql("SELECT * FROM transaction ORDER BY date DESC", engine)
...
def load_users():
    engine = init_connection()
    return pd.read_sql('SELECT id, name FROM "user" ORDER BY id', engine)
...
with tab_dashboard:
    ...
    with col7:
        df_parcelas = df_transaction[
            df_transaction["installment_total"].notna() &
            (df_transaction["installment_current"] < df_transaction["installment_total"])
        ]
        parcelas_futuras = (
            df_parcelas["value"] * (df_parcelas["installment_total"] - df_parcelas["installment_current"])
        ).sum()
            ...
            if submit:
                ...
                with engine.begin() as conn:
                    ...
                    sql_recount = text("""
                        ...
                        FROM transaction
                        WHERE establishment = :est AND category_id IS NOT NULL AND category_id NOT IN (8, 10)
